...
zero_weeks <- read.csv("est_counts_genes_kallisto_LizzySet1.csv")
head(zero_weeks)
metaData<- read.csv('0Weeks.csv', header = TRUE, sep = ",")
write.table(metaData, file = "sample_info1.csv", sep = ',', col.names = T, row.names = FALSE, quote = F)

countsData <- read.csv('est_counts_genes_kallisto_LizzySet1.csv', header = TRUE, sep = ",")
head(countsData)
...
# read in counts data
counts_data <- read.csv('COUNTSDATA.csv')
head(counts_data)


# read in sample info
colData <- read.csv('sample_info1.csv')
head(colData)
...
rld <- rlog(dds)
plotPCA(rld,intgroup = "dexamethasone")
