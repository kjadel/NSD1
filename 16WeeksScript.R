...
zero_weeks <- read.csv("est_counts_genes_kallisto_LizzySet3.csv")
head(zero_weeks)
metaData<- read.csv('16WeeksSampleInfo.csv', header = TRUE, sep = ",")
write.table(metaData, file = "16WeeksSampleInfo.csv", sep = ',', col.names = T, row.names = FALSE, quote = F)

countsData <- read.csv('est_counts_genes_kallisto_LizzySet3.csv', header = TRUE, sep = ",")
head(countsData)
write.table(countsData, file = "16WeeksCounts_Data.csv", sep = ',', col.names = T, row.names = T, quote = F)


# Step 1: preparing count data ----------------

# read in counts data
counts_data <- read.csv('16WeeksCounts_Data.csv')
head(counts_data)


# read in sample info
colData <- read.csv('16WeeksSampleInfo.csv')
head(colData)
...
write.csv(as.data.frame(res), 
          file="16WeeksCondition_treated_results.csv")

resFilt <- res[which(res$padj < 0.05 & abs(res$log2FoldChange) > 1), ]
write.csv(resFilt, file="16WeeksDEResultsFiltered.csv")
